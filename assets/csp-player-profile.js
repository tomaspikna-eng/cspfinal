...
function avatarHtml(profile,className="mini-avatar"){const name=profile.full_name||"Hráč",avatar=safeMediaUrl(profile.avatar_url);return avatar?`<span class="${className}"><img src="${esc(avatar)}" alt="${esc(name)}"></span>`:`<span class="${className}">${esc(initials(name))}</span>`}
function normalizedPlayerPlan(value){
  const plan=String(value||"free").trim().toLowerCase();
  return plan==="pro_plus"?"pro_plus":plan==="pro"?"pro":"free";
}
function sidebar(profile){
  const plan=normalizedPlayerPlan(profile.plan),home=playerPlanHome(plan);
  const active=id=>PAGE===id?" active":"";
  const link=(id,icon,label,href)=>`<a class="nav-link${active(id)}" href="${href}"><span class="nav-icon">${icon}</span>${label}</a>`;
  const createDisabled=plan!=="pro_plus";
  const create=(icon,label,href)=>`<a class="nav-link plan-create" data-create-feature href="${href}"${createDisabled?' aria-disabled="true"':""}><span class="nav-icon">${icon}</span>${label}</a>`;
  const nav=[
    link("home","⌂","Domov","/"),
    ...
    create("◆","Vytvoriť ligu","/ligy/"),
    create("▦","Vytvoriť udalosť","/udalost/")
  ].join("");
  return`<aside id="profile-sidebar" class="sidebar"><button class="mobile-sidebar-close" type="button" data-profile-menu-close aria-label="Zavrieť profilové menu">×</button><a class="brand" href="${home}">${logo()}<span class="brand-copy"><b>Connect</b><strong>Sports Pro</strong></span></a><nav class="nav" aria-label="Hlavná navigácia">${nav}</nav><div class="side-spacer"></div><div class="smart-card"><span>SMART SYSTEM</span><p>Tvoj šport. Tvoje dáta. Tvoj progres.</p></div><div class="side-user">${avatarHtml(profile)}<div><b>${esc(profile.full_name||"Hráč")}</b><span>${esc(planLabel(plan))} účet</span></div></div></aside>`;
}
function shell(profile,content){
  const [label,title]=PAGE_META[PAGE],plan=normalizedPlayerPlan(profile.plan);
  return`<div class="shell plan-${plan.replace("_","-")}">${sidebar(profile)}<div class="sidebar-backdrop" data-profile-menu-backdrop aria-hidden="true"></div><main class="workspace"><header class="topbar"><button class="mobile-menu-toggle" type="button" data-profile-menu-toggle aria-controls="profile-sidebar" aria-expanded="false" aria-label="Otvoriť profilové menu"><span></span></button><span class="mobile-brand"><img src="/assets/csp-player-logo.png" alt="CSP"></span><div class="topbar-title"><span>${label}</span><b>${title}</b></div><div class="topbar-tools"><button class="notification-button" type="button" data-csp-notifications-button aria-label="Notifikácie">🔔</button><div class="data-state"><i></i><span>Dáta zo Supabase</span></div></div></header><div class="stage">${content}</div></main></div>`;
}
function authState(error=false){ROOT.innerHTML=`<section class="auth-screen"><div class="auth-card">${logo()}<h1>${error?"Profil sa nepodarilo načítať":"Prihlásenie je potrebné"}</h1><p>${error?"Obnov stránku alebo skontroluj pripojenie.":"Tieto stránky zobrazujú dáta aktuálne prihláseného hráča."}</p>${error?"":'<a class="button gold" href="/login/?returnTo=/profil/">Prihlásiť sa</a>'}</div></section>`}
...
function playerHero(profile,disciplines){const name=profile.full_name||"Hráč",plan=planLabel(profile.plan),primary=disciplines.find(item=>item.is_primary)||disciplines[0],joined=dateValue(profile.created_at),cover=safeMediaUrl(profile.cover_url);const coverStyle=cover?` style="background-image:linear-gradient(90deg,rgba(5,6,5,.2),rgba(5,6,5,.8)),url('${safeCssUrl(cover)}')"`:"";return`<section class="panel hero public-content"><div class="hero-cover ${cover?"has-image":""}"${coverStyle}></div>${avatarHtml(profile,"avatar")}<div class="identity"><span class="eyebrow"><i class="live-dot"></i> HRÁČSKY PROFIL</span><div class="name-row"><h1>${esc(name)}</h1><span class="plan-badge ${plan==="FREE"?"free":""}">${plan}</span></div><p><b>${esc(String(profile.role||"hráč").toUpperCase())}</b><i></i><span>${esc(primary?[primary.sport,primary.discipline].filter(Boolean).join(" · "):"Športový profil")}</span>${joined?`<i></i><span>Člen od ${MONTHS[joined.getMonth()]} ${joined.getFullYear()}</span>`:""}</p></div><div class="hero-actions"><button class="button ghost" data-share>Zdieľať</button><a class="button gold" href="/profil/moj-profil/">Môj profil</a></div></section>`}
...
function bindProfileMobileMenu(){const sidebar=document.getElementById("profile-sidebar"),backdrop=document.querySelector("[data-profile-menu-backdrop]"),toggle=document.querySelector("[data-profile-menu-toggle]");if(!sidebar||!backdrop||!toggle)return;const setOpen=open=>{sidebar.classList.toggle("open",open);backdrop.classList.toggle("open",open);backdrop.setAttribute("aria-hidden",String(!open));toggle.setAttribute("aria-expanded",String(open));document.body.classList.toggle("profile-menu-open",open)};toggle.addEventListener("click",()=>setOpen(!sidebar.classList.contains("open")));document.querySelectorAll("[data-profile-menu-close]").forEach(button=>button.addEventListener("click",()=>setOpen(false)));backdrop.addEventListener("click",()=>setOpen(false));sidebar.querySelectorAll(".nav a").forEach(link=>link.addEventListener("click",()=>setOpen(false)));window.addEventListener("keydown",event=>{if(event.key==="Escape")setOpen(false)});window.addEventListener("resize",()=>{if(window.innerWidth>780)setOpen(false)})}
function attachInteractions(planValue,client=null){
  const plan=normalizedPlayerPlan(planValue),isFree=plan==="free";
  bindProfileMobileMenu();
  ...
  document.querySelectorAll("[data-share]").forEach(button=>button.addEventListener("click",async()=>{try{if(navigator.share)await navigator.share({title:document.title,url:location.href});else await navigator.clipboard.writeText(location.href)}catch(error){if(error.name!=="AbortError")console.warn(error)}}));
  document.querySelectorAll("[data-scoreboard]").forEach(link=>{if(isFree)link.addEventListener("click",event=>{event.preventDefault();alert("Spustenie tréningu je dostupné v pláne PRO. Toto je náhľad funkcionality.")})});
...
async function init(){
  try{
    ...
    ROOT.innerHTML=shell(profile,content);attachInteractions(profile.plan,client);
