-- Connect Sports Pro / Club Manager
-- Reservation backend: public booking layer + admin layer
-- Applied to Supabase project csp-staging on 2026-07-30.
...
create table if not exists public.reservations (
  ...
  source text not null default 'public'
    check (source in ('public','admin','profile','phone','walk_in')),
  public_note text,
  internal_note text,
...
create table if not exists public.reservation_blocks (
  ...
  block_type text not null default 'manual'
    check (block_type in ('manual','service','tournament','private_event','closed')),
  title text not null,
  note text,
  created_by uuid references public.profiles(id) on delete set null,
  created_at timestamptz not null default now(),
...
create index if not exists reservations_club_time_idx
  on public.reservations(club_id, starts_at, ends_at);
create index if not exists reservations_station_time_idx
  on public.reservations(station_id, starts_at, ends_at);
create index if not exists reservations_public_token_idx
  on public.reservations(public_token);
create index if not exists reservation_blocks_club_time_idx
  on public.reservation_blocks(club_id, starts_at, ends_at);
create index if not exists reservation_blocks_station_time_idx
  on public.reservation_blocks(station_id, starts_at, ends_at);
...
as $$
  select exists (
    ...
    where c.id = p_club_id
      and (c.owner_id = auth.uid() or p.is_admin)
  );
$$;
...
begin
  if not exists (
    ...
    where s.id = new.station_id and s.club_id = new.club_id
  ) then
  ...
  if new.venue_id is not null and not exists (
    ...
    where v.id = new.venue_id and v.club_id = new.club_id
  ) then
  ...
  if exists (
    select 1
    from public.reservation_blocks b
...
create trigger touch_reservation_settings
...
create trigger touch_reservation_opening_hours
before update on public.reservation_opening_hours
for each row execute function public.touch_reservation_updated_at();

drop trigger if exists touch_reservations on public.reservations;
create trigger touch_reservations
before update on public.reservations
for each row execute function public.touch_reservation_updated_at();

drop trigger if exists touch_reservation_blocks on public.reservation_blocks;
create trigger touch_reservation_blocks
before update on public.reservation_blocks
for each row execute function public.touch_reservation_updated_at();
...
create policy reservation_settings_admin_all
...
on public.reservation_opening_hours
for all to authenticated
using (public.can_manage_club(club_id))
with check (public.can_manage_club(club_id));
...
create policy reservation_blocks_admin_all
on public.reservation_blocks
for all to authenticated
using (public.can_manage_club(club_id))
with check (public.can_manage_club(club_id));
  ...
  slots as (
    select
      generate_series(
        ...
        ((p_day + coalesce(h.closes_at,time '22:00')) at time zone h.tz)
          - make_interval(mins => h.slot_minutes),
        make_interval(mins => h.slot_minutes)
      ) as slot_start,
      h.slot_minutes
    from hours h
    where coalesce(h.is_closed,false) = false
  )
  select
    st.id,
    st.name,
...
begin
  ...
  if found and not v_settings.public_booking_enabled then
    raise exception 'Public booking is disabled';
  end if;

  if p_starts_at < now() + make_interval(
    mins => coalesce(v_settings.min_notice_minutes,60)
  ) then
    raise exception 'Reservation is too soon';
  end if;

  if p_starts_at > now() + make_interval(
    days => coalesce(v_settings.booking_horizon_days,30)
  ) then
    raise exception 'Reservation is outside booking horizon';
  end if;

  if extract(epoch from (p_ends_at-p_starts_at))/60
      < coalesce(v_settings.min_duration_minutes,60) then
    raise exception 'Reservation is too short';
  end if;

  if extract(epoch from (p_ends_at-p_starts_at))/60
      > coalesce(v_settings.max_duration_minutes,240) then
  ...
  select
    ...
    p_ends_at,
    case
      when coalesce(v_settings.requires_confirmation,true)
        then 'pending'
      else 'confirmed'
    end,
    'public',
  ...
  return query
  select v_row.id, v_row.public_token, v_row.status;
end;
$$;
...
begin
  ...
  select coalesce(cancellation_notice_minutes,120)
  into v_notice
  from public.reservation_settings
  where club_id = v_res.club_id;

  if v_res.starts_at < now() + make_interval(mins => coalesce(v_notice,120)) then
    return false;
  end if;
...
grant execute on function public.get_public_booking_calendar(uuid,date)
  to anon, authenticated;
grant execute on function public.create_public_reservation(
  uuid,uuid,timestamptz,timestamptz,text,text,text,integer,text
) to anon, authenticated;
grant execute on function public.get_public_reservation(uuid)
  to anon, authenticated;
grant execute on function public.cancel_public_reservation(uuid)
  to anon, authenticated;
...
create index if not exists club_manager_live_sessions_reservation_idx
  on public.club_manager_live_sessions(reservation_id);
...
revoke all on function public.validate_reservation_relations()
  from public, anon, authenticated;
revoke all on function public.validate_block_conflicts()
  from public, anon, authenticated;
revoke all on function public.touch_reservation_updated_at()
  from public, anon, authenticated;
