-- Player profile location and club affiliation.
alter table public.profiles
  add column if not exists club_name text;

alter table public.profiles
  drop constraint if exists profiles_club_name_length,
  add constraint profiles_club_name_length
    check (club_name is null or char_length(club_name) <= 120);

comment on column public.profiles.club_name is
  'Player club affiliation shown in player profile settings and public profile.';
...
begin
  ...
  if exists (
    ...
    where k not in (
      'full_name','bio','avatar_url','city','country_code','club_name',
  ...
  set
    ...
    club_name = case when p_patch ? 'club_name'
      then nullif(trim(p_patch->>'club_name'),'') else p.club_name end,
