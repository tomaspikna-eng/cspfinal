...
begin
  ...
  if v_tournament.id is null then
    insert into public.tournaments(
      ...
      v_event.location_text,p_groups_count,p_advance_count,'draft',
      jsonb_build_object(
      ...
      v_event.city,v_event.visibility,v_event.registration_enabled,
      v_event.registration_deadline,v_event.max_participants,v_event.id
  ...
  for v_registration in
    select distinct on (lower(trim(coalesce(nullif(r.full_name,''),r.nickname))))
      trim(coalesce(nullif(r.full_name,''),r.nickname)) as player_name,
      r.user_id
    from public.event_preregistrations r
    where r.event_id=v_event.id
      and trim(coalesce(nullif(r.full_name,''),r.nickname))<>''
    order by lower(trim(coalesce(nullif(r.full_name,''),r.nickname))),
             (r.user_id is not null) desc, r.created_at asc
  loop
    update public.tournament_players tp
    set user_id=coalesce(tp.user_id,v_registration.user_id)
    where tp.tournament_id=v_tournament.id
      and (
        (v_registration.user_id is not null and tp.user_id=v_registration.user_id)
