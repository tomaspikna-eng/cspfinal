-- Live Station View for Club Manager reservations.
alter table public.training_sessions
  add column if not exists station_id uuid references public.stations(id) on delete set null;

create index if not exists training_sessions_station_live_idx
  on public.training_sessions(station_id,status)
  where station_id is not null and status in ('active','paused');
...
begin
  ...
  select
    ...
    case
      ...
      when lm.id is not null or lt.id is not null then 'occupied'
  ...
  left join lateral (
    ...
    limit 1
  ) rr on lm.id is null and lt.id is null
