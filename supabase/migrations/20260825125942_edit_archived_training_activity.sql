...
begin
  ...
  set display_name = case player_slot when 1 then v_name1 when 2 then v_name2 else display_name end,
      player_id = case when player_slot=1 then auth.uid() else player_id end
...
grant execute on function public.edit_my_training_activity(uuid,text,text,text,text,integer,integer,timestamptz,integer) to authenticated;
