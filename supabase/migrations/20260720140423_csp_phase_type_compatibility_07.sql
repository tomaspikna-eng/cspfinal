-- Align tournament phase identifiers used by the current CSP frontend with the database.
...
begin
  ...
  if ph.id is null then
    raise exception 'Phase not found or access denied';
  end if;

  if ph.phase_type not in ('rr', 'round_robin') then
    raise exception 'Phase is not round robin';
  end if;
  ...
  if normalized_next_type is null then
    raise exception 'Invalid next phase type';
  end if;
  ...
  for q in
    select * from jsonb_array_elements(coalesce(p_qualifiers, '[]'::jsonb))
  loop
    if not exists (
      select 1
  ...
  insert into public.tournament_phases (
    ...
    ph.tournament_id,
    next_num,
    normalized_next_type,
    'active',
    jsonb_build_object('source_phase_id', p_phase_id),
    now()
  )
  returning id into next_id;

  update public.tournaments
  set current_phase_id = next_id
  where id = ph.tournament_id;
...
revoke execute on function public.complete_round_robin_phase(uuid,jsonb,text)
  from public, anon;
grant execute on function public.complete_round_robin_phase(uuid,jsonb,text)
  to authenticated;
