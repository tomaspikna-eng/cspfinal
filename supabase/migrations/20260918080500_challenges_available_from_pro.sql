-- CSP 2026-09-18 — Challenges available from PRO plan upward.
-- FREE remains without active challenge access; PRO and PRO+ can read/join/progress.

drop policy if exists challenge_seasons_pro_plus_select on public.challenge_seasons;
drop policy if exists challenge_seasons_pro_select on public.challenge_seasons;
create policy challenge_seasons_pro_select
...
drop policy if exists challenges_pro_select on public.challenges;
create policy challenges_pro_select
