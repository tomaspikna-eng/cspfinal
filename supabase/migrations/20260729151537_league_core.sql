...
create trigger trg_league_rounds_updated_at before update on public.league_rounds
for each row execute function public.set_league_updated_at();
create trigger trg_league_matches_updated_at before update on public.league_matches
for each row execute function public.set_league_updated_at();
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    ...
    where l.id = p_league_id and (
      l.owner_id = auth.uid()
      or exists (select 1 from public.league_admins la where la.league_id=l.id and la.user_id=auth.uid() and la.role in ('owner','admin','scorer'))
...
declare
  v_format text; v_players uuid[]; v_count integer; v_slots uuid[];
  v_round integer; v_match integer; v_leg integer; v_i integer;
  v_p1 uuid; v_p2 uuid; v_round_id uuid; v_total integer := 0; v_tmp uuid;
begin
  if not public.can_manage_league(p_league_id) then raise exception 'Not authorized to generate this league schedule'; end if;
  select competition_format into v_format from public.leagues where id=p_league_id for update;
  if v_format is null then raise exception 'League not found'; end if;
  select array_agg(id order by coalesce(seed,2147483647),joined_at,id) into v_players
  from public.league_players where league_id=p_league_id and status in ('registered','active');
  ...
  v_slots := v_players;
  if mod(v_count,2)=1 then v_slots:=array_append(v_slots,null::uuid); v_count:=v_count+1; end if;
  for v_leg in 1..case when v_format='round_robin_double' then 2 else 1 end loop
    for v_round in 1..(v_count-1) loop
      ...
      values(p_league_id,((v_leg-1)*(v_count-1))+v_round,case when v_leg=1 then 'Kolo '||v_round else 'Odveta '||v_round end)
      returning id into v_round_id;
      v_match:=0;
      for v_i in 1..(v_count/2) loop
        v_p1:=v_slots[v_i]; v_p2:=v_slots[v_count-v_i+1];
        if v_p1 is not null and v_p2 is not null then
          v_match:=v_match+1;
          if v_leg=2 then v_tmp:=v_p1; v_p1:=v_p2; v_p2:=v_tmp; end if;
          insert into public.league_matches(league_id,round_id,round_number,leg_number,match_number,player1_id,player2_id)
          values(p_league_id,v_round_id,((v_leg-1)*(v_count-1))+v_round,v_leg,v_match,v_p1,v_p2);
          v_total:=v_total+1;
        end if;
      end loop;
      v_tmp:=v_slots[v_count];
      for v_i in reverse v_count..3 loop v_slots[v_i]:=v_slots[v_i-1]; end loop;
      v_slots[2]:=v_tmp;
    end loop;
  end loop;
...
returns table (
  standing_position bigint, league_player_id uuid, user_id uuid, display_name text,
  played bigint, wins bigint, draws bigint, losses bigint,
  score_for bigint, score_against bigint, score_difference bigint, points numeric, form text[]
)
language sql stable security definer set search_path = public as $$
with eligible as (
  select lp.id,lp.user_id,lp.display_name from public.league_players lp
  join public.leagues l on l.id=lp.league_id
  where lp.league_id=p_league_id and lp.status in ('registered','active','withdrawn')
    and (l.visibility in ('public','unlisted') or public.can_manage_league(l.id) or lp.user_id=auth.uid())
), results as (
  select m.*,l.win_points,l.draw_points,l.loss_points,l.forfeit_win_points,l.forfeit_loss_points
  from public.league_matches m join public.leagues l on l.id=m.league_id
  where m.league_id=p_league_id and m.status in ('completed','forfeited')
), per_player as (
  select e.id league_player_id,e.user_id,e.display_name,
    count(r.id) played,
    count(*) filter(where r.winner_id=e.id) wins,
    count(*) filter(where r.result_type='draw') draws,
    count(*) filter(where r.result_type<>'draw' and r.winner_id is distinct from e.id) losses,
    coalesce(sum(case when r.player1_id=e.id then r.score1 else r.score2 end),0)::bigint score_for,
    coalesce(sum(case when r.player1_id=e.id then r.score2 else r.score1 end),0)::bigint score_against,
    coalesce(sum(case when r.result_type='draw' then r.draw_points when r.winner_id=e.id and r.result_type in ('forfeit','walkover') then r.forfeit_win_points when r.winner_id=e.id then r.win_points when r.result_type in ('forfeit','walkover') then r.forfeit_loss_points else r.loss_points end),0)::numeric points
  from eligible e left join results r on e.id in (r.player1_id,r.player2_id)
  group by e.id,e.user_id,e.display_name
), recent_form as (
  select e.id,coalesce(array_agg(x.outcome order by x.completed_at desc) filter(where x.rn<=5),array[]::text[]) form
  from eligible e left join lateral (
    select m.completed_at,case when m.result_type='draw' then 'D' when m.winner_id=e.id then 'W' else 'L' end outcome,
...
select standing_position,league_player_id,user_id,display_name,played,wins,draws,losses,score_for,score_against,score_difference,points,form
...
create policy leagues_public_read on public.leagues for select using (visibility in ('public','unlisted') and status<>'draft' or owner_id=auth.uid() or public.can_manage_league(id));
create policy leagues_owner_insert on public.leagues for insert to authenticated with check (owner_id=auth.uid());
create policy leagues_manager_update on public.leagues for update to authenticated using (public.can_manage_league(id)) with check (public.can_manage_league(id));
create policy leagues_manager_delete on public.leagues for delete to authenticated using (public.can_manage_league(id));
create policy league_admins_read on public.league_admins for select to authenticated using (public.can_manage_league(league_id) or user_id=auth.uid());
create policy league_admins_manage on public.league_admins for all to authenticated using (public.can_manage_league(league_id)) with check (public.can_manage_league(league_id));
create policy league_players_public_read on public.league_players for select using (exists(select 1 from public.leagues l where l.id=league_id and (l.visibility in ('public','unlisted') and l.status<>'draft' or public.can_manage_league(l.id) or user_id=auth.uid())));
create policy league_players_manage on public.league_players for all to authenticated using (public.can_manage_league(league_id)) with check (public.can_manage_league(league_id));
create policy league_rounds_public_read on public.league_rounds for select using (exists(select 1 from public.leagues l where l.id=league_id and (l.visibility in ('public','unlisted') and l.status<>'draft' or public.can_manage_league(l.id))));
create policy league_rounds_manage on public.league_rounds for all to authenticated using (public.can_manage_league(league_id)) with check (public.can_manage_league(league_id));
create policy league_matches_public_read on public.league_matches for select using (exists(select 1 from public.leagues l where l.id=league_id and (l.visibility in ('public','unlisted') and l.status<>'draft' or public.can_manage_league(l.id))));
create policy league_matches_manage on public.league_matches for all to authenticated using (public.can_manage_league(league_id)) with check (public.can_manage_league(league_id));
