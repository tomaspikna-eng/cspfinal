...
begin
  ...
  if v_match.status = 'completed' then
    if v_match.score1 = p_score1
       and v_match.score2 = p_score2
       and v_match.winner_id is not distinct from p_winner_id then
      return v_match;
    end if;
    raise exception 'Match is already completed with a different result';
  end if;
  if v_match.status = 'forfeited' then
