...
as $$
  select coalesce(
    (
      select case
        when p.role in ('club'::public.profile_role, 'organization'::public.profile_role)
          then '/profil-ul/'
        else '/profil/'
      end
...
comment on function public.get_my_profile_route() is
  'Canonical authenticated profile frontend route. player -> /profil/; club and organization -> /profil-ul/. Subscription plan is intentionally ignored.';
