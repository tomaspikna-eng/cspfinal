...
as $function$
  ...
  recent_training as (
    select
      ...
      coalesce(
        (
          select tsp.player_slot
          from public.training_session_participants tsp
          where tsp.training_session_id = ts.id
            and tsp.player_id = (select id from me)
          order by tsp.player_slot
          limit 1
        ),
        1
      )::integer as my_slot,
      coalesce(
        ts.winner_name,
        case
          when coalesce(ts.live_scores[1],0) > coalesce(ts.live_scores[2],0) then ts.player_names[1]
          when coalesce(ts.live_scores[2],0) > coalesce(ts.live_scores[1],0) then ts.player_names[2]
          else null
        end
      ) as effective_winner_name
  ...
  training_summary as (
    select
      ...
      count(*) filter (
        where effective_winner_name is not null
          and player_names[my_slot] is not null
          and lower(effective_winner_name) = lower(player_names[my_slot])
      )::integer as wins,
      count(*) filter (
        where effective_winner_name is not null
          and player_names[my_slot] is not null
  ...
  select jsonb_build_object(
    ...
    'summary', (
      select jsonb_build_object(
        ...
        'training_wins', ts.wins,
        'training_losses', ts.losses,
        'match_wins', ms.wins,
        'match_losses', ms.losses,
        'wins', ts.wins + ms.wins,
        'losses', ts.losses + ms.losses,
        'win_rate', case
          when ts.wins + ts.losses + ms.wins + ms.losses = 0 then 0
          else round((ts.wins + ms.wins)::numeric * 100 / (ts.wins + ts.losses + ms.wins + ms.losses), 1)
        end,
    ...
    'recent_trainings', coalesce((
      ...
      from (
        select
          ...
          effective_winner_name as winner_name,
