...
text,n=re.subn(r'  function injectStyles\(\)\{.*?\n  \}\n\n  function waitForTarget',inject_new,text,count=1,flags=re.S)
if n!=1:
    raise SystemExit('injectStyles patch failed')

render_new='''  function render(target,payload,publicView){
  ...
  async function init(){'''
text,n=re.subn(r'  function render\(target,payload,publicView\)\{.*?\n  \}\n\n  async function init\(\)\{',render_new,text,count=1,flags=re.S)
if n!=1:
    raise SystemExit('render patch failed')

social_path.write_text(text,encoding='utf-8')

profile=Path('profil/index.html')
html=profile.read_text(encoding='utf-8')
html=html.replace('/assets/csp-social.js?v=20260827-1','/assets/csp-social.js?v=20260915-seal-1')
profile.write_text(html,encoding='utf-8')
